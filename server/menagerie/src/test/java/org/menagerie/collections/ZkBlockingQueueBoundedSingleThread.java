...
public class ZkBlockingQueueBoundedSingleThread extends MenagerieTest {
    ...
    public void testOfferTimesOut() throws Exception{
        ...
        //offer timed
        boolean notAccepted = !testQueue.offer(testElement + "tooMuch", 500l, TimeUnit.MILLISECONDS);
        assertTrue("offer was incorrectly accepted!",notAccepted);
    }

    @Test(timeout = 1500l,expected = IllegalStateException.class)
    public void testAddToFullThrowsException() throws Exception{
        //fill the queue
        for(int i = 0;i<maxQueueSize;i++){
            testQueue.put(testElement+i);
        }
        ...
        //offer timed
        testQueue.add(testElement + "tooMuch");
    }

    @Test(timeout = 1000l)
    public void testAvailableCapacity() throws Exception{
        assertEquals("remainingCapacity() is incorrect!",maxQueueSize,testQueue.remainingCapacity());
        testQueue.put(testElement);
        assertEquals("remainingCapacity() is incorrect!",maxQueueSize-1,testQueue.remainingCapacity());
        testQueue.remove();
        assertEquals("remainingCapacity() is incorrect!",maxQueueSize,testQueue.remainingCapacity());
    }
    ...
    public void testTimedPollSuccessfullyReturns() throws Exception{
        ...
        String retString = testQueue.poll(5000, TimeUnit.MILLISECONDS);
