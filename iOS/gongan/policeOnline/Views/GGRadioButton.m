...
-(void)defaultInit{
    ...
    [GGRadioButton registerInstance:self];
}
#pragma mark - Manage Instances
//添加选中的RadioButton
+(void)registerInstance:(GGRadioButton *)radioButton{
    if(!rb_instances){
...
-(void)handleButtonTap:(id)sender{
    ...
    [GGRadioButton buttonSelected:self];
}
...
+(void)buttonSelected:(GGRadioButton *)radioButton{
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            GGRadioButton *button = [rb_instances objectAtIndex:i];
