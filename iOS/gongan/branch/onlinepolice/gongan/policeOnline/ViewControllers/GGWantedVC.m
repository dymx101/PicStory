...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([column isKindOfClass:[GGWantedCategory class]]) {
        GGWantedCategory  *category = [_mutableKeys objectAtIndex:indexPath.row];
    ...
    {
        DLog(@"进入通缉令详情");
        GGWanted * wanted = [_mutableKeys objectAtIndex:indexPath.row];
        GGWebVC *vc = [[GGWebVC alloc] init];
        vc.urlStr = [NSString stringWithFormat:@"%@/mobile-clueInfo.rht?contentId=%lld", GGN_STR_PRODUCTION_SERVER_URL, wanted.ID];
        vc.naviTitleString = wanted.title;
        [self.navigationController pushViewController:vc animated:YES];
    }
